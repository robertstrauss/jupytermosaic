...
import { Tile, Mosaic } from './MosaicGroup'; Mosaic;
...
export class MosaicViewModel extends WindowedListModel {
    ...
    _widgetRenderer (index: number): Widget {
        console.log('widget renderer!', index, this.tiles[this.tileOrder[index]].isPlaceholder(), this.tiles[this.tileOrder[index]].isDisposed);
        // (this.tiles[this.tileOrder[index]] as any).placeHolder = false;
        // console.log('widget renderer!', index, this.tiles[this.tileOrder[index]].isPlaceholder(), this.tiles[this.tileOrder[index]].isDisposed);
        return this.tiles[this.tileOrder[index]]
    }
    ...
    estimateWidgetSize = (index: number): number => {
        return 0;/*
        const id = this.tileOrder[index];
        const cell = this.tiles[id];
        if (!cell) {
